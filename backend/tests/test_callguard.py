...
class TestSignalWeights:
    ...
    def test_signal_weight_accumulation(self):
        ...
        # Duplicate signals currently accumulate in rule-based scoring
        assert multiple_score == single_score * 3
...
class TestIntegrationScenarios:
    ...
    def test_bank_scam_scenario(self):
        ...
        assert response.level in ["medium", "high"]
        assert response.safe_script is not None
        assert response.metadata.get("primary_signal") == "verification_code_request"
    ...
    def test_tech_support_scam_scenario(self):
        ...
        assert response.metadata.get("primary_signal") == "remote_access_request"
